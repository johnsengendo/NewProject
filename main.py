...
# Applying custom CSS for professional network theme styling
st.markdown("""
<style>
    .main-header {
...
# Setting configurations and directory paths
DATA_DIR = "Data"
...
# Displaying main header with professional styling
st.markdown("""
...
# Configuring sidebar with styling
st.sidebar.markdown("## Digital Twin Control Panel")
st.sidebar.markdown("---")

# Initializing session state for parameters if they do not exist
...
if apply_settings:
    ...
    st.success("✅ Applying digital twin settings successfully!")
...
# Implementing main logic for training or inference
need_training = (
...
if need_training and (st.session_state.dt_params_applied or not any([key.startswith('current_') for key in st.session_state.keys()])):
    ...
    try:
        ...
        # Assessing performance for inference
